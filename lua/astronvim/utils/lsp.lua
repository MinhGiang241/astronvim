...
local setup_handlers = user_opts("lsp.setup_handlers", {function(server, opts)
...
M.formatting = user_opts("lsp.formatting", {
    format_on_save = {
        enabled = true
    },
    disabled = {}
    ...
    if not config_avail or not config.default_config then
        ...
        if server_definition.cmd then
            require("lspconfig.configs")[server] = {
                default_config = server_definition
            }
        end
    end
...
M.on_attach = function(client, bufnr)
    ...
    if client.supports_method "textDocument/codeLens" then
        add_buffer_autocmd("lsp_codelens_refresh", bufnr, {
            ...
            callback = function()
                if not M.has_capability("textDocument/codeLens", {
                    bufnr = bufnr
                }) then
    ...
    if client.supports_method "textDocument/formatting" and not tbl_contains(M.formatting.disabled, client.name) then
        ...
        if autoformat.enabled and
            ...
            (tbl_isempty(autoformat.ignore_filetypes or {}) or not tbl_contains(autoformat.ignore_filetypes, filetype)) then
            add_buffer_autocmd("lsp_auto_format", bufnr, {
                events = "BufWritePre",
                desc = "autoformat on save",
                callback = function()
                    if not M.has_capability("textDocument/formatting", {
                        bufnr = bufnr
                    }) then
                    ...
                    if autoformat_enabled and ((not autoformat.filter) or autoformat.filter(bufnr)) then
                        vim.lsp.buf.format(extend_tbl(M.format_opts, {
                            bufnr = bufnr
                        }))
                    end
                end
    ...
    if client.supports_method "textDocument/documentHighlight" then
        add_buffer_autocmd("lsp_document_highlight", bufnr, {{
            ...
            callback = function()
                if not M.has_capability("textDocument/documentHighlight", {
                    bufnr = bufnr
                }) then
    ...
    utils.set_mappings(user_opts("lsp.mappings", lsp_mappings), {
        buffer = bufnr
    })

    for id, _ in pairs(astronvim.lsp.progress) do -- clear lingering progress messages
        if not next(vim.lsp.get_active_clients {
            id = tonumber(id:match "^%d+")
        }) then
            astronvim.lsp.progress[id] = nil
        end
    end
...
function M.config(server_name)
    ...
    local lsp_opts = extend_tbl(server, {
